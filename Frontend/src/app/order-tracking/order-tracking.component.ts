...
@Component({
  ...
  imports: [ReactiveFormsModule, MatTooltipModule,
    MatButtonModule, CommonModule, LoaderComponent, HttpClientModule],
...
debugger
if (this.orderNo) {
    this.loaderService.addRequest(this.orderService.orderParcelTracking(this.orderNo)).subscribe({
      next: (data: OrderModel[]) => {
        ...
        const lastDoneSeq = data
          .filter(item => item.seqStatus === 'REALIZED' || item.seqStatus === 'ISSUE')
...
hasAnyIssue(): boolean {
  return this.orderSteps.some(step => this.getStatus(step.no)?.toUpperCase() === 'ISSUE');
}

// Check if all buttons are 'REALIZED' (no 'ISSUE' allowed)
areAllRealized(): boolean {
  return this.orderSteps.every(step => this.getStatus(step.no) === 'REALIZED');
}
...
if (!this.isSeqNoButtonActive(seqNo)) return;
  ...
  fetchStepDetails(seqNo: string) {    
    this.orderService.orderParcelTrackingBy_Seq(this.orderNo?.toString() || '', Number(seqNo)).subscribe({
      next: (data) => {
